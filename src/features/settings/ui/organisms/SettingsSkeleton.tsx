...
function FieldSkeleton({ labelWidth }: { labelWidth: string }) {
  return (
    <div className="flex flex-col gap-2">
      <Skeleton animationType="none" className={`${BAR} h-3.5 ${labelWidth}`} />
      <Skeleton
        animationType="none"
        className={`${BAR} h-12 w-full rounded-full`}
      />
    </div>
  );
}
    ...
    <>
      ...
      <div className="mt-4 w-full">
        <div className="px-6 pt-6">
          <Skeleton animationType="none" className={`${BAR} h-7 w-40`} />
        </div>
        <div className="bg-surface rounded-2xl p-6">
          <div className="mb-6 flex flex-col gap-2">
            <Skeleton animationType="none" className={`${BAR} h-5 w-48`} />
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div className="col-span-full flex flex-col gap-4 rounded-2xl border border-dashed p-4 sm:flex-row sm:items-center">
              <Skeleton
                animationType="none"
                className={`${BAR} size-24 shrink-0 rounded-2xl`}
              />
              <div className="flex flex-1 flex-col gap-3">
                <Skeleton animationType="none" className={`${BAR} h-4 w-32`} />
                <Skeleton
                  animationType="none"
                  className={`${BAR} h-3.5 w-56`}
                />
                <Skeleton
                  animationType="none"
                  className={`${BAR} h-10 w-32 rounded-full`}
                />
              </div>
            </div>
            ...
            <div className="col-span-full mt-8 flex flex-col gap-2">
              <Skeleton animationType="none" className={`${BAR} h-5 w-52`} />
              <Skeleton animationType="none" className={`${BAR} h-3.5 w-72`} />
            </div>
      ...
      <div className="mt-4 w-full">
        ...
        <div className="bg-surface flex flex-wrap items-center justify-between gap-4 rounded-2xl p-6">
          <div className="flex flex-col gap-2">
            <Skeleton
              animationType="none"
              className={`${BAR} h-6 w-24 rounded-full`}
            />
            <Skeleton animationType="none" className={`${BAR} h-3.5 w-72`} />
          </div>
          <Skeleton
